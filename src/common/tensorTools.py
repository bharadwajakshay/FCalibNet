...
def convSO3NTToSE3(RMat, Trans):
    ...
    Trans = torch.transpose(Trans.unsqueeze(1),2,1)
...
def calculateInvRTTensorWhole(RT):
...
def applyTransformationOnTensor(ptCloud, transform):
    ...
    if len(ptCloud.shape)==3:
        assert ptCloud.shape[-1] == 3
        organizedPtCld = False
    elif len(ptCloud.shape)==4:
        assert ptCloud.shape[-1] == 3
        organizedPtCld = True
    else:
        exit(-1)
    ...
    for axis in range(3):
        translation = transform[:,axis,3].repeat((1,transformed_points.shape[-1]))
        transformed_points[:,axis,:] = transformed_points[:,axis,:] + transform[:,axis,3].unsqueeze(dim=1)
        
    if organizedPtCld:
        transformed_points = transformed_points.transpose(1,2)
        ptCloud = transformed_points.reshape((imageSize[0], imageSize[1], imageSize[2], imageSize[3]))
